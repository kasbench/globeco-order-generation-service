...
class HealthCheck:
    ...
    async def get_health_status(
        ...
        external_degraded = (
            "external_services" in checks and 
            checks["external_services"]["status"] == "degraded"
        )
...
async def readiness_probe(settings: Settings = Depends(get_settings)) -> HealthStatus:
    ...
    try:
        ...
        if health_status.status == "unhealthy":
            ...
            external_degraded = (
                "external_services" in health_status.checks and
                health_status.checks["external_services"]["status"] == "degraded"
            )
            database_healthy = (
                "database" in health_status.checks and
