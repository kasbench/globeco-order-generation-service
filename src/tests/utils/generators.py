...
def generate_test_securities(count: int = 10) -> List[Dict[str, Any]]:
    ...
    companies = [
        ...
        ("NVIDIA Corp", "NVDA"), ("Berkshire Hathaway", "BRK.B"), ("Johnson & Johnson", "JNJ"),
        ("JPMorgan Chase", "JPM"), ("Procter & Gamble", "PG"), ("Walmart Inc", "WMT"),
        ("Coca-Cola Co", "KO"), ("IBM Corp", "IBM"), ("Intel Corp", "INTC"),
    ]
    ...
    for i in range(count):
        ...
        securities.append({
            ...
            "marketCap": random.uniform(1_000_000_000, 2_000_000_000_000),  # $1B to $2T
            "currency": "USD",
        })
...
def generate_random_portfolio(
    ...
    portfolio_id = f"{random.randint(1000000000000000000000000, 9999999999999999999999999):024x}"
...
def generate_investment_model(
    ...
    if model_name is None:
        model_types = ["Conservative", "Moderate", "Aggressive", "Growth", "Value", "Balanced"]
    ...
    for i in range(num_positions):
        if i == num_positions - 1:
            ...
            if max_target <= min_target:
                ...
                target = Decimal(str(random.uniform(
                    float(min_target), 
                    float(max_target)
    ...
    non_zero_targets = [(sec, tgt) for sec, tgt in zip(selected_securities, targets) if tgt > 0]
    ...
    for security, target in non_zero_targets:
        ...
        low_drift = Decimal(str(random.uniform(0.01, float(high_drift)))).quantize(Decimal("0.001"))
