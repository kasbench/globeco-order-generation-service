...
def assert_optimization_valid(
    ...
    """
    assert len(quantities) == len(prices) == len(target_percentages) == len(low_drifts) == len(high_drifts), \
        "All input lists must have the same length"
    
    # Check that all quantities are non-negative integers
    for i, qty in enumerate(quantities):
        assert isinstance(qty, int) and qty >= 0, f"Quantity {i} must be non-negative integer: {qty}"
    ...
    for i, (qty, price, target, low_drift, high_drift) in enumerate(
        ...
        assert (lower_bound - tolerance) <= position_value <= (upper_bound + tolerance), (
...
def assert_transactions_valid(transactions: List[Dict[str, Any]]) -> None:
    ...
    for i, transaction in enumerate(transactions):
        ...
        assert transaction["transactionType"] in ["BUY", "SELL"], (
