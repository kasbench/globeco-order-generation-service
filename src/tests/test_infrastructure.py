...
from src.tests.utils.generators import generate_test_securities, generate_security_prices
...
class TestTestingInfrastructure:
    ...
    def test_optimization_assertion_utility(self):
        ...
        assert_optimization_valid(
            quantities, prices, target_percentages, market_value,
            low_drifts, high_drifts, tolerance=Decimal("1000")  # Large tolerance for this test
        )
        
        # Test constraint violation with clearly invalid quantities
        invalid_quantities = [500, 200, 0]  # First position way too large (500*50 = 25000 = 100% of portfolio)
        with pytest.raises(AssertionError):
            assert_optimization_valid(
                invalid_quantities, prices, target_percentages, market_value,
                low_drifts, high_drifts, tolerance=Decimal("0.01")
            )
...
class TestMathematicalFixtures:
    ...
    def test_simple_optimization_problem_fixture(self, simple_optimization_problem):
        ...
        required_keys = [
            "current_quantities", "target_percentages", "prices", 
            "market_value", "low_drifts", "high_drifts"
        ]
